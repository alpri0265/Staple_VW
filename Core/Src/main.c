...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if (GPIO_Pin == STOP_BTN_Pin) {
        // Фізична зупинка — миттєво на БУДЬ-якому фронті (безпека), незалежно
        // від того, чи це справжнє натискання, чи коротка наводка від мотора.
        motor_emergency_stop();
        // "Логічна" подія СТОП (яка скидає озброєння бузера дотяжки й перемикає
        // ERROR/MAIN екран, main.c:510-525) тут навмисно НЕ виставляється:
        // коротка наводка від STEP-імпульсів мотора інакше миттєво розброювала б
        // бузер ще до того, як кут фізично дійде до цілі. Її виставляє
        // input_debounce_tick() лише після підтвердженого утримання кнопки
        // ≥DEBOUNCE_MS — це відфільтровує наводки, залишаючи фізичну
        // зупинку мотора миттєвою.
        //
        // Маскуємо лінію EXTI0 одразу — дребезг/наведення від мотора під час руху
        // (STEP-імпульси DM556/NEMA34) інакше можуть викликати шторм переривань на
        // найвищому пріоритеті й "заморозити" TIM3/головний цикл. Розмаскування —
        // в input_debounce_tick(), коли кнопка стабільно відпущена.
